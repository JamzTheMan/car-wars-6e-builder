...
import { Card as CardType, CardArea, CardType as CardTypeEnum, canCardTypeGoInArea } from '@/types/types';
  ...
  return (
    <>
      <div
        ...
        className={`relative w-32 h-48 rounded-lg shadow-lg overflow-hidden transition-transform ${isDragging ? 'opacity-50' : ''
      ...
      {isPreviewOpen && (
        <div className="fixed inset-0 bg-transparent backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={closePreview}>
          <div className="bg-white bg-opacity-85 rounded-lg overflow-hidden shadow-2xl max-w-3xl max-h-[90vh] relative" onClick={e => e.stopPropagation()}>
              ...
              <div className="flex flex-col md:flex-row gap-6">
                ...
                <div className="w-full md:w-1/2 flex-shrink-0">
                  <div className="mb-4">
                    ...
                    <p className="text-gray-600"><span className="font-medium">Type:</span> {card.type}</p>
                    {card.subtype && <p className="text-gray-600"><span className="font-medium">Subtype:</span> {card.subtype}</p>}
                    <p className="text-gray-600">
                      <span className="font-medium">Cost:</span> {(card.buildPointCost ?? 0) + (card.crewPointCost ?? 0)} points
                    </p>                    {/* Conditionally show description if available */}
                    {card.description && (
                      <div className="mt-4">
                  ...
                  {isInCollection && (
                    <div className="mt-4">
                      ...
                      {(() => {
                        ...
                        const vehicleLocations = [CardArea.Front, CardArea.Back, CardArea.Left, CardArea.Right];
                        const vehicleButtons = availableAreas.filter(area => vehicleLocations.includes(area));
                        const otherButtons = availableAreas.filter(area => !vehicleLocations.includes(area));
                          ...
                          return (
                            <div className="flex flex-col gap-2">
                              {availableAreas.map((area) => (
                                ...
                                >
                                  <FontAwesomeIcon icon={faClone} />
                                  {area === 'gearupgrade' ? 'Gear & Upgrades' : area.charAt(0).toUpperCase() + area.slice(1)}
                                </button>
                              ))}
