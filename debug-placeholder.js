...
function canAddCardToDeck(card, deckCards) {
  ...
  if (card.type === CardType.Gear && gearCardsInDeck.length > 0) {
    ...
    gearCardsInDeck.forEach(c => {
      ...
      console.log(`Is placeholder? Deck card: ${c.imageUrl.includes('Blank_') || c.imageUrl.includes('placeholders/')} | New card: ${card.imageUrl.includes('Blank_') || card.imageUrl.includes('placeholders/')}`);
    });
    ...
    const hasSameImage = gearCardsInDeck.some(c => 
